...
model1(c(7, 1.5), sim1)
measure_distance <- function(mod, data) {diff <- data$y - model1(mod, data) 
                                         sqrt(mean(diff ^ 2))} 
...
ggplot(models, aes(a1, a2)) +  geom_point(data = filter(models, rank(dist) <= 10), size = 4, color = "red") +  geom_point(aes(colour = -dist))
...
yeet <- coef(sim1_mod) 
ggplot(sim1, aes(x, y)) +  geom_point(size = 2, color = "grey30") +  geom_abline(intercept = yeet[1], slope = yeet[2])

## Exercise
sim1a <- tibble(x = rep(1:10, each = 3),  y = x * 1.5 + 6 + rt(length(x), df = 2))
sim1a_mod <- lm(y ~ x, data = sim1a)
yeet1 <- coef(sim1a_mod)
ggplot(sim1a, aes(x,y)) + geom_point(size = 2, color = "blue") + geom_abline(intercept = yeet1[1], slope = yeet[2])
measure_distance <- function(mod, data) {diff <- data$y - model1(mod, data)  
                                         mean(abs(diff))}
best1 <- optim(c(0,0), measure_distance, data = sim1a)
best1$par
yeet1
ggplot(sim1a, aes(x,y)) + geom_point(size = 2, color = "blue") + geom_abline(intercept = c(yeet1[1],best1$par[1]), slope = c(yeet1[2],best1$par[2]))
...
yeet <- coef(sim1_mod2)
ggplot(sim1, aes(x,y)) + geom_point(size = 2, color = "blue") + geom_abline(yeet[1], yeet[2]) + coord_cartesian(xlim = c(0,10), ylim = c(0,30))
grid2 <- sim1 %>% 
  data_grid(x)
grid2
grid2 <- grid2 %>% 
...
sim1b
ggplot(sim1b, aes(resid)) + geom_freqpoly(binwidth = 0.5)
ggplot(sim1b, aes(x,resid)) + geom_ref_line(h = 0) + geom_point()
...
## INTERACTIONS BETWEEN TWO CONTINUOUS
...
grid <- sim4 %>% 
  data_grid(x1 = seq_range(x1, 5),
            x2 = seq_range(x2, 5)) %>%  
  gather_predictions(mod1, mod2) 
seq_range(c(0.0123, 0.923423), n = 5) ## creates five equally spaced values between 0.0123 and 0.923423 also including 0.0123 and 0.923423
seq_range(c(0.0123, 0.923423), n = 5, pretty = TRUE)
x1 <- rcauchy(100) 
seq_range(x1, n = 5) 
seq_range(x1, n = 5, trim = 0.10) ## trim is used to trim out a certain percentage of the values that are on the edges
seq_range(x1, n = 5, trim = 0.25)
seq_range(x1, n = 5, trim = 0.50) 

x2 <- c(0, 1)
seq_range(x2, n = 5) 
seq_range(x2, n = 5, expand = 0.10) ## expand expands the range by a certain percentage 
...
sim5 <- tibble(x = seq(0, 3.5 * pi, length = 50),  y = 4 * sin(x) + rnorm(length(x)))
...
df <- tribble(~x, ~y,  
              ...
              NA, 10)
mod <- lm(y ~ x, data = df) 
mod <- lm(y ~ x, data = df, na.action = na.exclude) ## USED TO SUPPRESS WARNINGS
nobs(mod) ## reports the number of observations that were used
