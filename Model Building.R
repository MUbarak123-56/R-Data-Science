...
diamonds2 %>% 
  ...
  select(pred, price, carat:table,x:z) %>% 
  arrange(price)
...
daily %>% 
  filter(resid < - 100)
daily %>%  ggplot(aes(date, resid)) +  geom_ref_line(h = 0) +  geom_line(color = "grey50") +  geom_smooth(se = FALSE, span = 0.20) 

## SEASONAL SATURDAY EFFECT
daily %>% 
  filter(wday == "Sat") %>% 
  ggplot(aes(date, n)) + geom_point() + geom_line() + scale_x_date(NULL, date_breaks = "1 month", date_labels = "%b")
term <- function(date) {
...
daily %>% 
  filter(wday == "Sat") %>% 
  ggplot(aes(date, n, color = term)) + geom_point(alpha = 1/3) + geom_line() + scale_x_date(NULL, date_breaks = "1 month", date_labels = '%b')
daily %>%  
...
daily %>%  
  ...
  ggplot(aes(date, resid)) +  geom_hline(yintercept = 0, size = 2, color = "white") +  geom_line()
...
## TIME OF THE YEAR: ALTERNATIVE APPROACH
library(splines)
